...
dunstify -a "Battery_Notify" -t 3000 "$output" -u normal -i $DIR/angel.png -h string:x-canonical-private-synchronous:"$msgID_1"
}
...
get_discharging() {
	...
	if [[ $capacity -lt $cap ]]; then
			output="Current Battery Capacity:$capacity"
			dunstify -a "Battery_Notify" -t 3000 "$output" -u critical -i $DIR/death.png -h string:x-canonical-private-synchronous:"$msgID_1"
	fi
}
